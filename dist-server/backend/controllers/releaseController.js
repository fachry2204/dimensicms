...
export const createRelease = async (req, res) => {
    try {
        ...
        const [result] = await pool.query('INSERT INTO releases (user_id, type, title, status) VALUES (?, ?, ?, ?)', [userId, type, 'Untitled Release', 'pending'] // Initially pending or we can add 'draft' status if needed. PRD says 'pending' after submit. Let's assume it's pending but incomplete.
        );
        res.status(201).json({
...
export const updateRelease = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const { title, label, p_line, c_line, genre, language, version, upc, release_date, previous_distribution, brand_new, previously_released } = req.body;
        // Check ownership
        const [releases] = await pool.query('SELECT * FROM releases WHERE id = ? AND user_id = ?', [id, userId]);
        ...
        if (p_line) {
            updateFields.push('p_line = ?');
            updateValues.push(p_line);
        }
        if (c_line) {
            updateFields.push('c_line = ?');
            updateValues.push(c_line);
        }
        if (genre) {
        ...
        if (req.file) {
            ...
            updateValues.push(coverPath);
        }
        if (updateFields.length > 0) {
            updateValues.push(id);
            await pool.query(`UPDATE releases SET ${updateFields.join(', ')} WHERE id = ?`, updateValues);
        }
...
export const addTrack = async (req, res) => {
    try {
        ...
        const userId = req.user.id;
        const { title, track_number, isrc, genre, explicit, composer, lyricist, lyrics } = req.body;
        ...
        await pool.query(`INSERT INTO tracks (release_id, track_number, title, audio_path, audio_clip_path, isrc, genre, explicit, composer, lyricist, lyrics)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [id, track_number, title, audioPath, clipPath, isrc, genre, explicit === 'true', composer, lyricist, lyrics]);
...
export const getRelease = async (req, res) => {
    try {
        ...
        const [releases] = await pool.query(query, params);
        if (releases.length === 0) {
            return res.status(404).json({ message: 'Release not found' });
        }
        const [tracks] = await pool.query('SELECT * FROM tracks WHERE release_id = ? ORDER BY track_number ASC', [id]);
